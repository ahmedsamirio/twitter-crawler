...
def stream_from_users(twitter_api, user, tweets_per_user, friends_per_user, mongo_db, depth):
    ...
    5. mongo_db (str): Database name
    6. depth (int): Current depth while calling the function
    ...
    if depth < limit_depth:
        ...
        if friends and followers:
            ...
            # randomly select n users from friends and followers merged lists
            users = np.random.choice(friends+followers, friends_per_user)

            for user in users:
                if user['id'] not in collected_users_list:
                    _, _, _ = stream_from_users(twitter_api, user, tweets_per_user, friends_per_user,
                                          mongo_db, depth+1)
    return len(collected_users_list), tweets_count, time.time() - start_time
...
def save_tweets_to_sql(twitter_api, tweets, user, db, db_cursor):
...
def save_tweets_json(tweets, user):
    print(user['id'])
...
def save_user_to_sql(user, db, db_cursor):
